--script to set up stored procedure sp_dumptrans in database
--saptools
--version 1.0.2
--date 04 Oct 2012 
--changes: 
--1.0.1 fix issue that cause sp to not run in jobscheduler 
--1.0.2 fix failure to execute when called from sp_thresholdaction
...
go
create procedure sp_dumptrans (@sapdb_name varchar (256))
as
begin
  ...
  begin
    begin
      ...
      select @dumpcmd = 'dump transaction ' || @sapdb_name || ' to "' || @curr_logdev || '" with compression = 101'
      insert into saptools..dump_history(DBName,Type,Command,StartTime,EndTime,RC,Status) VALUES (@sapdb_name,'LOG',@dumpcmd,@starttime,NULL,NULL,'R')
      commit
      exec(@dumpcmd)
